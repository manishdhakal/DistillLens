...
def add_model_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--lm-head", type=str, default="lm_head")
...
def add_hp_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--fkl-coeff", type=float, default=1.0)
    group.add_argument("--rkl-coeff", type=float, default=1.0)
    group.add_argument("--lens-coeff", type=float, default=1.0)
    group.add_argument("--mse-coeff", type=float, default=0.0)
    group.add_argument('--warmup-iters', type=float, default=0.1,
                       help='percentage of data to warmup on (.1 = 10% of all '
    ...
    group.add_argument("--teacher-logit-lens", nargs="+", type=int, default=None, help="logit lens layers for teacher model")
    group.add_argument("--student-logit-lens", nargs="+", type=int, default=None, help="logit lens layers for student model")
...
def get_args():
    ...
    #     args.save,
    #     "inter_kd" if args.teacher_logit_lens is not None else "kd",
    #     args.ckpt_name,
    #     args.type,
    #     (f"s{args.seed}-e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}-N{args.n_gpu}-NN{args.n_nodes}") + \
    ...
    #         (f"-fkl{args.fwd_kl_ratio}" if args.fwd_kl_ratio is not None else "") + \
    #         (f"-mp{args.model_parallel_size}" if args.model_parallel > 0 else "") + \
    #         (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}" if args.peft == "lora" else "") + \
    #         args.save_additional_suffix
    #     )
