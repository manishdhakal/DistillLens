...
class TeacherLensProj(nn.Module):
    def __init__(self, t_h_dim, n_lenses):
        super().__init__()
        self.t_lenses = nn.ModuleList([nn.Linear(t_h_dim, t_h_dim, bias=True) for _ in range(n_lenses)])


class StudentLensProj(nn.Module):
    def __init__(self, s_h_dim, n_lenses):
        super().__init__()
        self.s_lenses = nn.ModuleList([nn.Linear(s_h_dim, s_h_dim, bias=True) for _ in range(n_lenses)])


def get_tunedlens_proj(args):
    n_teacher_lenses = len(args.teacher_logit_lens)
    n_student_lenses = len(args.student_logit_lens)
    t_h_dim = AutoConfig.from_pretrained(args.teacher_model_path).hidden_size
    s_h_dim = AutoConfig.from_pretrained(args.model_path).hidden_size
    return TeacherLensProj(t_h_dim, n_teacher_lenses), StudentLensProj(s_h_dim, n_student_lenses)