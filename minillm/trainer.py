...
from transformers import AutoTokenizer, GenerationConfig, mpu
...
from .utils import get_log_probs, get_rev_kl, significant, get_kl
...
class PPOTrainer:
    ...
    def __init__(self, args, tokenizer: AutoTokenizer, reward_fn: callable, ds_config):
        ...
        if args.model_parallel:
            if mpu.get_data_parallel_rank() == 0:
                print(
                    " > number of parameters on model parallel rank {}: {}M".format(
                        mpu.get_model_parallel_rank(),
                        int(sum([p.nelement() for p in self.model.parameters()]) / 1e6),
                    ),
                    flush=True,
                )
        else:
        ...
        self.model, self.opt, self.scheduler = self.setup_ds(
            self.model, self.opt, self.scheduler
        )

        self.tokenizer = tokenizer
        self.store = PPORolloutStorage(
            self.tokenizer.pad_token_id, self.args.seed_ppo + self.dp_rank
        )
    ...
    def get_model_inputs(
        ...
        batch = {"input_ids": tokens, "attention_mask": attention_mask}
    ...
    def forward_model(self, batch: dict[str, torch.Tensor]):
        outputs = self.model(
            **batch, return_dict=True, use_cache=False, output_hidden_states=True
        )
        return outputs
    ...
    def compute_logits_and_log_probs(
        ...
        outputs = model_cls(**batch, return_dict=True, use_cache=False)
        ...
        if return_logprobs:
            logprobs = get_log_probs(
                ...
                mask,
                inf_mask,
                model_parallel=self.args.model_parallel,
            )
                ...
                for it, batch in enumerate(self.train_dataloader):
                    if self.lm_pipeline is not None:
                        try:
                            ...
                            save_rank(
                                f"Another lm epoch, lm epochs: {lm_epochs}",
                    ...
                    outputs = self.forward_model(input_batch)
                    logits = outputs.logits
                    ...
                    # Intermediate losses
                    # TODO:
                    t_logit_lens_layers = [40, 44]
                    logit_lens_layers = [11, 12]
                    inter_coef = 0.1
                    lm_head_name = "lm_head"
                    fkl_ratio = 0.5
                    if t_logit_lens_layers and logit_lens_layers:
                        n_layers = len(logit_lens_layers)
                        B, L, V = logits.size()
                        with torch.no_grad():
                        ...
                        hidden_states = tuple(
                            outputs.hidden_states[i] for i in logit_lens_layers
                        )
                        hidden_states = torch.stack(hidden_states, dim=1).view(
                            B * n_layers, L, -1
                        )
                        t_hidden_states = tuple(
                            t_outputs.hidden_states[i] for i in t_logit_lens_layers
                        )
                        t_hidden_states = torch.stack(t_hidden_states, dim=1).view(
                            B * n_layers, L, -1
                        )

                        lm_head = getattr(self.model.module.base_model, lm_head_name)
                        t_lm_head = getattr(self.teacher_model, lm_head_name)

                        hidden_logits = lm_head(hidden_states)
                        with torch.no_grad():
                            t_hidden_logits = t_lm_head(t_hidden_states)

                        inf_mask = torch.isinf(hidden_logits) | torch.isinf(
                            t_hidden_logits
                        )

                        # Mask the padding and query tokens
                        mask = self.get_mask(input_batch["input_ids"])
                        mask[:, : batch.query_tensors.size(1)] = 0
                        mask = mask.repeat(n_layers, 1).view(B * n_layers, L)

                        fkl_loss = get_kl(
                            t_hidden_logits, hidden_logits, inf_mask, mask
                        )
                        rkl_loss = get_kl(
                            hidden_logits, t_hidden_logits, inf_mask, mask
                        )
                        inter_loss = fkl_ratio * fkl_loss + (1 - fkl_ratio) * rkl_loss
                    else:
                        inter_loss = 0.0

                    loss = (
                        rl_loss + self.args.lm_coef * pt_loss + inter_coef * inter_loss
                    )
                    stats["inter_loss"] = inter_loss
                    ...
                    if (
                        ...
                        and (
                            (
                                self.global_iter_count < 10000
                                and (self.global_iter_count % 1000 == 0)
                            )
                            or self.global_iter_count % self.args.save_interval == 0
                        )
                    ):
                        self.save()
                    ...
                    if (
                        ...
                        and (
                            (
                                self.global_iter_count < 1000
                                and (self.global_iter_count % 100 == 0)
                            )
                            or (self.global_iter_count % self.args.eval_interval == 0)
                        )
                    ):
                        self.evaluate()
                    ...
                    mid_log_step = (
                        self.args.gradient_accumulation_steps // self.args.mid_log_num
                    )
                    ...
                    if (
                        self.global_iter_count % self.args.log_interval == 0
                        and self.iter_count % self.args.gradient_accumulation_steps == 0
                    ):
                        logging_stats = {
                            k: v
                            / (
                        ...
                        log_str = get_log(
                            logging_stats,
                            logging_stats.get("elapsed_time", 0)
                            * self.args.gradient_accumulation_steps,
                        )
                        print_rank("*" * 100)
                        print_rank(log_str)
                    ...
                    # end
                    if (
                        self.global_iter_count >= self.total_steps
                        or self.sampler.epochs >= self.args.epochs
                    ):
                        if self.global_iter_count >= self.total_steps:
                            print_rank(
                                "Reached total steps {}/{}".format(
                                    self.global_iter_count, self.total_steps
                                )
                            )
                        else:
                            print_rank(
                                "Reached data epochs {}/{}".format(
                                    self.sampler.epochs, self.args.epochs
                                )
                            )
                        self.save()
    ...
    def evaluate_ppo(self):  # noqa: C901
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                batch, _ = self.eval_pipeline.move_to_device(
                    batch, no_model_batch, self.device
                )
                gen_out = self.generate(
                    **batch, return_dict_in_generate=True, output_scores=True
                )
                ...
                mask = mask[
                    :, input_ids.size(1) - 1 : input_ids.size(1) + gen_ids.size(1) - 1
                ]
                lens = torch.sum(mask, dim=-1)

                teacher_rewards = self.reward_fn(input_ids, gen_ids)[
                    "rewards"
                ]  # \log p(y_t | y_{<t}, x)
                _, logprobs = self.compute_logits_and_log_probs(
                    input_ids, gen_ids, inf_mask=inf_mask, base="base"
                )  # \log q_{\theta}(y_t | y_{<t}, x)
            ...
            all_rev_kl = all_gather(
                all_rev_kl, dim=0, world_size=self.dp_world_size, group=self.dp_group
            )
            stats["rev_kl"] = all_rev_kl.mean()
            all_lens = all_gather(
                all_lens, dim=0, world_size=self.dp_world_size, group=self.dp_group
            )
            ...
            if get_rank() == 0:
                ...
                response_texts = self.tokenizer.batch_decode(
                    full_ids[:, self.eval_pipeline.max_prompt_length :],
    ...
    def evaluate_pt(self):
        ...
        all_kd_losses = []
        for batch in tqdm(
            self.eval_lm_dataloader, desc="LM Evaluation", disable=(not get_rank() == 0)
        ):
        ...
        eval_pt_loss = (
            all_gather(
                all_pt_losses, dim=0, world_size=self.dp_world_size, group=self.dp_group
            )
            .mean()
            .item()
        )
        ...
        eval_lm_loss = (
            all_gather(
                all_lm_losses, dim=0, world_size=self.dp_world_size, group=self.dp_group
            )
            .mean()
            .item()
        )
        ...
        eval_kd_loss = (
            all_gather(
                all_kd_losses, dim=0, world_size=self.dp_world_size, group=self.dp_group
            )
            .mean()
            .item()
        )
    ...
    def save(self, directory: Optional[str] = None):
        ...
        if self.args.model_parallel:
            ...
            if get_rank() == 0:
                self.model.module.base_model.save_pretrained(
                    ckpt_dir, safe_serialization=False
                )
    ...
    def save_evals(
        self, preds, results, response_texts, directory: Optional[str] = None
    ):
        """Creates a checkpoint of the optimizer, scheduler and model"""
        """Creates checkpoint of optimizer, scheduler and a model"""
        base_ckpt_path = directory or self.args.save
